...
export function PostCommentComposer({ postId, token, onSuccess, className }: PostCommentComposerProps) {
  const [comment, setComment] = useState('')
  const inputId = `post-comment-${postId}`

  const commentMutation = useMutation({
    mutationFn: () => addComment(token, postId, comment.trim()),
    onSuccess: () => {
      setComment('')
      onSuccess()
    },
  ...
  return (
    <div className={cn('relative mt-4 max-w-xl', className)}>
